...
### 自定义事件
```javascript
document.body.addEventListener('animate', function(event) {
  if (!event.detail) return;
  for (let key in event.detail) this.style[key] = event.detail[key];
});

let event = new CustomEvent('animate', {
  detail: {
    opacity: 0,
    transition: '.3s'
  }
...
💡 若项目需要兼容 IE9-，请直接使用 jQuery 1.x，而不是自行编写兼容性代码。
