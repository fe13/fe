...
class TouchPad {
 ...
  onTouchStart(event) { console.log('触控开始', event.clientX, event.clientY); }
  
  onTouchMove(event) { console.log('触控移动', event.clientX, event.clientY); }
  
  onTouchEnd(event) { console.log('触控结束', event.clientX, event.clientY); }
}
```
