...
## 文档宽高
理论上，文档宽高应该是 `documentElement.scrollWidth` `documentElement.scrollHeight`，实际上...
```javascript
let 文档宽度 = Math.max(
...
* `elem.clientWidth` 元素内容**可视**宽度，包含 `padding`，不包含滚动条
* `elem.clientHeight` 元素内容**可视**高度，包含 `padding`，不包含滚动条
* `elem.scrollWidth` 元素内容**实际**宽度，包含因滚动而不可见的部分，包含 `padding`，不包含滚动条
* `elem.scrollHeight` 元素内容**实际**高度，包含因滚动而不可见的部分，包含 `padding`，不包含滚动条
...
`EventTarget.prototyoe.dispatchEvent()` 分派/触发事件

## 继承关系
通过添加以下 `Node.prototype.ctors()` 方法查看特定节点对象的继承关系。 
```javascript
