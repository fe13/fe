...
## 查找元素
`element.getElementsByTagName()` 方法用于获取指定标签名的元素集合。💡 不要漏了 `s`。
```javascript
...
`element.getElementsByClassName()` 方法用于获取指定类名的元素集合。多个类名以空格隔开。
```javascript
...
```javascript
document.querySelector('.champions .warrior')
```

`element.querySelectorAll()` 方法用于获取匹配指定选择器的元素集合。没有则返回 `[]`。
```javascript
...
```javascript
for (let warrior of warriors) console.log(warrior)
```
